...
try: 
    from transformers.trainer import amp, smp_forward_backward, smp, xm, met
except:
    pass
...
class TrainOutputMulti(NamedTuple):
    global_step: int
...
class AdaTrainer(Trainer):
    ...
    def _maybe_log_save_evaluate(self, tr_loss_dict, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            for loss_key in tr_loss_dict.keys():
                ...
                tr_loss_dict[loss_key] -= tr_loss_dict[loss_key]
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        return {'loss': loss.detach() / self.args.gradient_accumulation_steps,
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    tr_loss_step_dict = self.training_step(model, inputs)

                for loss_key in tr_loss_step_dict.keys():
        ...
        for loss_key in train_loss_dict.keys():
            metrics['train_' + loss_key] = train_loss_dict[loss_key]
        ...
        return TrainOutputMulti(self.state.global_step, train_loss_dict, metrics), model

    def save_final_checkpoint(self, model, trial=None, metrics=None, MY_PREFIX_CHECKPOINT_DIR="final_ckpt_backup"):
        # In all cases, including ddp/dp/deepspeed, self.model is always a reference to the model we
        # want to save except FullyShardedDDP.
        # assert unwrap_model(model) is self.model, "internal model should be a reference to self.model"

        # Save model checkpoint
        checkpoint_folder = f"{MY_PREFIX_CHECKPOINT_DIR}-{self.state.global_step}"
